const mocks = {
  ...
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InRlc3QiLCJpc0FkbWluIjpmYWxzZSwiaWF0IjoxNjcyODQ0NTU4fQ.BBcChRTiZLfVYJHCgfp9xIG6Vf0qbJroDty1GMPl5MI",
  "user": { "username": "test", "firstName": "test", "lastName": "test", "email": "t@1.com", "isAdmin": false, "applications": [] },
  "jobs": [
    {
      "id": 200,
...
class JoblyApi {
  ...
  static getCurrentUser(logOut = false) {
    if (logOut) this.logout();
    let {token, user} = JSON.parse(localStorage.getItem('token')) || {};
    let data = JSON.parse(localStorage.getItem('users'));
    return {token: token, user: token ? data[user.username] : ''};
  }

  /** Get companies (filtered by name if not undefined) */

  static getCompanies(name) {
    if (name) {
  ...
  static getCompany(handle) {
    let result = {
      success: false,
      data: undefined
    };

    for(let c in mocks.companies){
      if (mocks.companies[c].handle === handle.toLocaleLowerCase()){
        result.success = true;
        result.data = mocks.companies[c];
      }
    }
    return result;
  }
  ...
  static getJobs(title, offset = 0, lim=10) {
    if (title) {
    ...
    let jobs = [];
    for(let i = offset; i < offset+lim; i++){
      jobs.push(mocks.jobs[i])
    }
    return jobs;
  }
  ...
  static async applyToJob(id) {
    let user = this.getCurrentUser().user;
    let apps = user['applications'];
    if(apps){
      if (apps.indexOf(id) === -1) apps.push(id);
    }else{
      apps = [id];
    }

    user['applications'] = apps;
    this.updateUser(JSON.parse(localStorage.getItem('users')), user)
    return apps;
  }
  ...
  static async updateUser(base, target){
    // localStorage.setItem('users', {...localstorage.getItem('users'), [data.username]: JSON.stringify({target})});
    localStorage.setItem('users', JSON.stringify({...base, [target.username]: target}))
  }

  static async setToken(username = ''){
    localStorage.setItem('token',
      JSON.stringify({
        token: username ? this.getToken() : '',
        user: username
      })
    );
  }
  ...
  static async login(data) {
    ...
    let user = users ? users[data.username.toLocaleLowerCase()] : false;
    let result = {
      success: false,
      errors: []
    };
  ...
  static async signup(data) {
    ...
    for(let key in data){
      if (data[key].trim().length === 0) {
        result.success = false;
        result.errors.push(`a valid ${key} is required`);
    }


    if (!users[username]){
      if (result.success){
        localStorage.setItem('users', JSON.stringify({...users, [data.username]: data}));
        this.setToken(data);
      }
    }else{
  ...
  static async saveProfile(username, data) {
    ...
    if (data['confirm password'] === user.password){
      user['first name'] = data['first name'];
      user['last name'] = data['last name'];
      user.email = data['email'];
      localStorage.setItem('users', JSON.stringify({...users, [username]:user}));
      this.setToken(user);
      process.success = true;
      process.status = ['saved'];
    }else{
      process.status = ['passwords do not match'];
    }
